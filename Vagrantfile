...
$s2memory               = ENV.fetch("S1_VAGRANT_MEMORY", "512")
$s3memory               = ENV.fetch("S1_VAGRANT_MEMORY", "512")
...
$s1virtualBoxDescription = ENV.fetch("S1_VAGRANT_VIRTUALBOXDESCRIPTION", "s1\nmemory: " + $s1memory + "\ncreated: " + $timeStamp)
$s2virtualBoxDescription = ENV.fetch("S2_VAGRANT_VIRTUALBOXDESCRIPTION", "s2\nmemory: " + $s2memory + "\ncreated: " + $timeStamp)
$s3virtualBoxDescription = ENV.fetch("S3_VAGRANT_VIRTUALBOXDESCRIPTION", "s3\nmemory: " + $s3memory + "\ncreated: " + $timeStamp)
$w1virtualBoxDescription = ENV.fetch("W1_VAGRANT_VIRTUALBOXDESCRIPTION", "w1\nmemory: " + $w1memory + "\ncreated: " + $timeStamp)
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "server1" do |subconfig|
    ...
    #subconfig.vm.network :private_network, ip: "10.0.2.21"
  ...
  config.vm.define "server2" do |subconfig|
    ...
    #subconfig.vm.network :private_network, ip: "10.0.2.22"
  ...
  config.vm.define "server3" do |subconfig|
    ...
    #subconfig.vm.network :private_network, ip: "10.0.2.23"
  ...
  config.vm.define "desktop" do |subconfig|
    ...
    #subconfig.vm.network :private_network, ip: "10.0.2.20"
